...
/**
 * /admin/analytics — Full analytics dashboard with tabs
 *
 * Tabs:
 * 1. Overview — Revenue, orders, customers, top products, low stock
 * 2. Cohorts — Customer retention heatmap by acquisition month
 * 3. Lifetime Value — CLV distribution, top customers, trends
 * 4. Funnels — Order-stage conversion funnel
 * 5. Categories — Revenue breakdown by product category
 */
...
import {
  ...
  GitBranch, DollarSign, Filter, PieChart as PieChartIcon,
} from "lucide-react";
import CohortsTab from "@/components/admin/analytics/CohortsTab";
import LtvTab from "@/components/admin/analytics/LtvTab";
import FunnelsTab from "@/components/admin/analytics/FunnelsTab";
import CategoriesTab from "@/components/admin/analytics/CategoriesTab";
...
type TabKey = "overview" | "cohorts" | "ltv" | "funnels" | "categories";

const TABS: { key: TabKey; label: string; icon: React.ReactNode }[] = [
  { key: "overview", label: "Overview", icon: <BarChart3 size={14} /> },
  { key: "cohorts", label: "Cohorts", icon: <GitBranch size={14} /> },
  { key: "ltv", label: "Lifetime Value", icon: <DollarSign size={14} /> },
  { key: "funnels", label: "Funnels", icon: <Filter size={14} /> },
  { key: "categories", label: "Categories", icon: <PieChartIcon size={14} /> },
];
...
export default function AnalyticsPage() {
  ...
  return (
    <div style={{ padding: "2rem" }}>
      ...
      {/* Tab bar */}
      <div style={{ display: "flex", gap: "0.25rem", background: "var(--surface)", border: "1px solid var(--gold-border)", borderRadius: "10px", padding: "4px", marginBottom: "2rem", overflowX: "auto" }}>
        {TABS.map((t) => (
          <button
            key={t.key}
            ...
            style={{
              ...
              color: tab === t.key ? "#0a0a0a" : "var(--muted)",
      ...
      {tab === "cohorts" && <CohortsTab period={period} />}
      {tab === "ltv" && <LtvTab />}
